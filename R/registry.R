...
register_extra_blocks <- function() {
  blockr.core::register_blocks(
    ...
    description = c(
      ...
      "Transform data with a plain R script (no wrapper function). Top-level assignments become controls: a factor renders a dropdown over its levels, intersect()/unique() and friends a multi-select over the columns or values the data has, a number a spin box, TRUE/FALSE a checkbox. Exports as idiomatic R with the current values written in.",
    ...
    guidance = c(
      ...
      paste(
        ...
        "again, or when another declaration reads it (that line is the pool",
        "feeding the knob below it).",
